...
async def _(event):
    ...
    if not pmpermit_sql.is_approved(chat_id):
        ...
        if event.is_private:
         ...
         ONE = ("__Okay. Your request has been registered. Do not spam my master's inbox.You can expect a reply if he doesn't choose to ignore your noob ass. He is a busy man, unlike you probably.__\n\n"
                "**⚠️ You will be blocked and reported if you spam!⚠️**\n\n"
                )
         TWO = ("**Go bother someone else. You have been blocked and reported until further notice.**")
         FOUR = ("Okay. My master has not seen your message yet.He usually responds to people,though idk about retarted ones.\n He'll respond when he comes back, if he wants to. There's already a lot of pending messages😶\n **Please do not spam unless you wish to be blocked and reported.**")
        ...
        async with borg.conversation(chat) as conv:
         ...
         if y == "1":
             ...
             if x == "1":
                 ...
                 await borg.send_message(chat, "`Lol Gey`")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
